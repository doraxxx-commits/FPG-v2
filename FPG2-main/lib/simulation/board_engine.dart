...
class BoardEngine {
  ...
  List<String> processDay({required List<Club> clubs, required List<Player> players}) {
    ...
    for (final club in clubs) {
      ...
      club.boardConfidence = (club.boardConfidence - delta).clamp(5, 100);
      ...
      if (club.boardConfidence <= 20 && club.managerTenureDays > 60) {
        if (_random.nextDouble() < 0.025 + club.boardPressure / 5000) {
          ...
          club.managerQuality = (club.managerQuality + _random.nextInt(15) - 7).clamp(30, 95);
          club.managerReputation = (club.managerReputation + _random.nextInt(11) - 5).clamp(20, 95);
