...
class _TeamScreenState extends State<TeamScreen> {
  ...
  void _teamBuilding(String action) {
    ...
    setState(() {
      if (action == 'BBQ') {
        _lockerRoomChemistry = (_lockerRoomChemistry + 8).clamp(0, 100);
        player.morale = (player.morale + 5).clamp(0, 100);
        player.fatigue = (player.fatigue + 4).clamp(0, 100);
        msg = 'Zorganizowałeś wspólnego grilla dla drużyny! Zgranie +8, Morale +5.';
      } else if (action == 'TALK') {
        _lockerRoomChemistry = (_lockerRoomChemistry + 4).clamp(0, 100);
