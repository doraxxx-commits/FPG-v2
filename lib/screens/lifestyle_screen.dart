...
class _LifestyleScreenState extends State<LifestyleScreen> {
  ...
  void _triggerActivity(String type) {
    ...
    setState(() {
      switch (type) {
        case 'DATE':
          _relationshipStatus = (_relationshipStatus + 12).clamp(0, 100);
          player.morale = (player.morale + 8).clamp(0, 100);
          player.fatigue = (player.fatigue + 5).clamp(0, 100);
          message = 'Spędziłeś miły wieczór z partnerką. Morale +8, Zmęczenie +5.';
          break;

        case 'REST':
          player.fatigue = (player.fatigue - 20).clamp(0, 100);
          player.fitness = (player.fitness + 10).clamp(0, 100);
          message = 'Zostałeś w domu i odpocząłeś przed meczem. Zmęczenie -20.';
          break;

        case 'PARTY':
          _relationshipStatus = (_relationshipStatus - 10).clamp(0, 100);
          player.morale = (player.morale + 15).clamp(0, 100);
          player.fatigue = (player.fatigue + 25).clamp(0, 100);
